...
The plugin SHALL include a `README.md` file documenting the plugin's purpose, installation steps, the `.worktreeinclude.yaml` configuration format, and the `configure` skill usage with examples.

#### Scenario: User reads README for setup guidance

- **WHEN** a user views the plugin's README
- **THEN** they find documentation covering: what the plugin does, how to install it, the config file format with examples, and how to use the `/worktree-bootstrap:configure` command
